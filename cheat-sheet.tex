...
\newcommand{\DNTIME}[1]{\textbf{\ooalign{N\cr D}}\textsf{TIME}(#1)}
\newcommand{\DSPACE}[1]{\textsf{DSPACE}(#1)}
\newcommand{\NSPACE}[1]{\textsf{NSPACE}(#1)}
\newcommand{\DNSPACE}[1]{\textbf{\ooalign{N\cr D}}\textsf{SPACE}(#1)}
...
\newthing{Ladner} If $\fat{P} \neq \fat{NP}$ then there eff. exists $L \in \fat{NP} \setminus \fat{P}$ that is not $\fat{NP}$-complete unter polynomial-time constructions.

\newthing{Sparse} $L \subseteq \Sigma^*$ is \emph{sparse} if there $\exists$ polynomial $p$ s.t. $|L \cap \Sigma^n| \leq p(n) \forallnatnum$.

\newthing{Mahaney} If $\fat{P} \neq \fat{NP}$. Then there exists no sparse language that is $\fat{NP}$-hard under $\preduce$.
...
\newthing{Nick's class} $k \in \mathbb{N}$. $\fat{NC}^k$ class of languages defined by c-polynomial uniform binary circuit families $\mathcal{C} = (C_n)_{n \in \mathbb{N}}$ with $db(C_n) \in \bigO{\log^k{n}} \forallnatnum$. $\fat{AC}^k$ for non-binary circuits.

\newthing{Oracle TM} TM with additional write-only, auto-advance output tape and special states $q_y$, $q_n$, $q_?$. For $O \subseteq \Sigma^*$, spontaneous transition from $q_?$ to $q_{w \stackrel{?}{\in} O}$.
\begin{itemize}
...
\newthing{Polynomial Turing reduction} $A$, $B$ problems. $A \ptreduce B$ if there ex. det. polyn.-time oracle TM $M$ with $A = L(M^B)$.
...
\begin{itemize}
  ...
  \item $\textsf{PIT} \in \textsf{co-}\fat{RP}$
  \item Monte-Carlo algorithms have uncertain correctness, Las Vegas algorithms have uncertain runtime.
  \item \enquote{Zero-error Probabilistic Polynomial time} $ = \fat{ZPP} = \fat{RP} \cap \textsf{co-}\fat{RP}$
...
\begin{itemize}
  \item $w \in L$, then Bob accepts input $w$ with prop. $\geq 1-2^{-|w|}$
  \item $w \not \in L$, then \underline{no $(A', B)$ exists} s.t. Bob accepts $w$ with probability $\geq 2^{-|w|}$
\end{itemize}
\begin{itemize}
